...
- **Inscripciones**: filtrar (estado/tipo/búsqueda), paginar, ver detalle con voucher, aprobar/rechazar (en grupos se aplica al grupo completo), **eliminar (mueve a hoja Eliminados con motivo; en grupos mueve a todos sus integrantes)**, exportar CSV. Selector de vista **"Todos" / "Grupos de 10"**. La hoja Inscripciones **no** tiene columna `MotivoRechazo` (solo existe en Eliminados). **Registro manual**: al elegir "Grupo de 10" se puede ingresar todo el grupo de una vez (representante + hasta 9 integrantes por línea `DNI,Nombres,App,Am`).
